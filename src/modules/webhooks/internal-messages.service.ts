...
interface SendMessageTransactionData {
  type: MessageType;
  content: string;
...
export class InternalMessagesService {
  ...
  sendMessageTransaction(
    ...
    userId: string,
    messageData: SendMessageTransactionData,
    ...
    return this.messageRepository.sendMessageTransaction(
      ...
      messageData,
      {
        lastMessageAt: new Date(conversationUpdate.lastMessageAt),
  ...
  updateTranscription(id: string, transcription: string) {
    return this.messageRepository.updateTranscription(id, transcription);
  }

  async getHistory(conversationId: string, take?: string) {
  ...
  async updateConversationId(messageIds: string[], newConversationId?: string) {
    let targetConversationId = newConversationId;

    if (!targetConversationId) {
      // Auto-resolve: find the message's current conversation, then find the last closed conversation of the same client
      const message = await this.messageRepository.findById(messageIds[0]);
      if (!message) {
        throw new Error(`Message ${messageIds[0]} not found`);
      }
    ...
    return this.messageRepository.updateConversationId(messageIds, targetConversationId);
  }
